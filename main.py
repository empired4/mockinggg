...
from config import API_ID, API_HASH, BOT_TOKEN, TMP_DIR
...
async def message_handler(client: Client, message: Message):
    ...
    if state == STATE_WAIT_MOCK_ID:
        ...
        if not any(m["id"] == mock_id for m in mocks):
        ...
        try:
            mock_data = await fetch_mock_details(auth_token, mock_id)
            html_file = generate_mock_html(mock_data)
            # Send file
            await message.reply_document(html_file, caption=f"{mock_data['name']} - Offline Mock Test")
            # Cleanup
